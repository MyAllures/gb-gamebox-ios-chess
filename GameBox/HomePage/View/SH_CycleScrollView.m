...
@interface SH_CycleScrollView () <UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
@implementation SH_CycleScrollView
...
- (void)initializePageControl {
    ...
    _pageControl.center = CGPointMake(CGRectGetWidth(_scrollView.frame)*0.5, CGRectGetHeight(_scrollView.frame)+10);
...
- (void)loadData {
    ...
    _datasourceImages = [_datasource numberOfCycleScrollView:self];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (page != _pageControl.currentPage) {
        if ([self.delegate respondsToSelector:@selector(cycleScrollView:didScrollToIndex:)]) {
            [self.delegate cycleScrollView:self didScrollToIndex:page];
        }
    }
...
- (void)singleTapGestureRecognizer:(UITapGestureRecognizer *)tapGesture {
    ...
    if ([self.delegate respondsToSelector:@selector(cycleScrollView:didSelectedAtIndex:)]) {
        [self.delegate cycleScrollView:self didSelectedAtIndex:self.isContinuous ? --page : page];
    }
}
